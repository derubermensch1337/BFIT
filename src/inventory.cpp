...
item make_item(const char *name, item_type type, uint8_t weight, uint8_t price) {
    ...
    if (name) {                                                     // make sure the user provided an input for the name.
        strncpy(new_item.name, name, sizeof(new_item.name) - 1);    // copy the input into the instanse of the item strukt.
        new_item.name[sizeof(new_item.name) - 1] = '\0';            // go back and manualy add a end of line terminator (incase the input was to long and got cut of).
    } 
